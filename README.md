...
client = SerpApi::Client.new(
  ...
  timeout: 5, # HTTP timeout in seconds, applied in persistent and non-persistent mode. (default: 120s)
  symbolize_names: true, # turn on/off JSON keys to symbols (default: on, more efficient)
  raise_on_search_error: true # raise when a 200 response carries an `error` field (default: true; see Search Archive API)
)
...
By default, `search_archive` returns the archived payload even when the original search recorded an `error` (for example a query that returned no results), so failed searches can still be inspected. Pass `raise_on_search_error: true` to raise a `SerpApiError` instead (see [issue #17](https://github.com/serpapi/serpapi-ruby/issues/17)):

```ruby
results = client.search_archive(search_id, raise_on_search_error: true)
```
